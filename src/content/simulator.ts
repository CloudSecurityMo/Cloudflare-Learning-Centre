...
export const STAGE_META: Record<StageId, { label: string; blurb: string }> = {
  ...
  ddos: { label: "DDoS Protection", blurb: "Always-on network-wide analysis for volumetric/protocol/L7 flood patterns." },
...
// Conceptual request-processing model — mirrors Cloudflare's documented
// security phase order: DDoS -> Custom Rules -> Rate Limiting -> Managed
// Rules -> Bot Fight Mode. This simulator combines Custom + Managed Rules
// into one "waf" stage, so Rate Limiting is modeled ahead of it. The actual
// evaluation order for a given zone depends on ruleset phases, product
// configuration, and plan — see developers.cloudflare.com/waf/feature-interoperability/
export const STAGE_ORDER: StageId[] = ["browser", "dns", "edge", "tls", "ddos", "rateLimit", "waf", "bot", "cache", "origin", "response"];
