...
export const NODE_DETAILS: Record<string, ArchitectureNodeDetail> = {
  ...
  "rate-limiting": {
    ...
    label: "Rate Limiting",
    category: "security",
    whereItSits: "After WAF Custom Rules but before WAF Managed Rules in Cloudflare's fixed phase order — evaluating request volume over time by a chosen key (IP, session, header).",
  ...
  "bot-management": {
    ...
    category: "security",
    whereItSits: "Evaluates client authenticity via fingerprinting/behavior. Bot Fight Mode is the last phase in Cloudflare's fixed order — after WAF Custom Rules, Rate Limiting, and WAF Managed Rules. (A Bot Management score, where available, is generated early enough to be referenced inside Custom Rule expressions.)",
    ...
    configuredBy: "Bot Fight Mode (Free) or Super Bot Fight Mode (Pro/Business) settings; Custom Rules referencing cf.bot_management.score require the Bot Management Enterprise add-on.",
